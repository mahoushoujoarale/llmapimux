...
func (m *Mux) OpenAIChatHandler() http.Handler {
	return &Handler{codec: &openaiChatCodec{}, router: m.router, auth: m.auth, stats: m.stats, reqMod: m.reqMod}
}
...
func (m *Mux) OpenAIResponsesHandler() http.Handler {
	return &Handler{codec: &openaiResponsesCodec{}, router: m.router, auth: m.auth, stats: m.stats, reqMod: m.reqMod}
}

// AnthropicHandler returns an http.Handler for Anthropic Messages inbound requests.
func (m *Mux) AnthropicHandler() http.Handler {
	return &Handler{codec: &anthropicCodec{}, router: m.router, auth: m.auth, stats: m.stats, reqMod: m.reqMod}
}

// GeminiHandler returns an http.Handler for Gemini GenerateContent inbound requests.
func (m *Mux) GeminiHandler() http.Handler {
	return &Handler{codec: &geminiCodec{}, router: m.router, auth: m.auth, stats: m.stats, reqMod: m.reqMod}
}
