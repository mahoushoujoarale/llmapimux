...
- Developer-to-system role mapping: `WithMapDeveloperToSystem` MuxOption causes outbound OpenAI Chat Completions requests to emit SystemPrompt as `"system"` role instead of `"developer"`. The IR consolidates the *leading run* of system and developer messages into `Request.SystemPrompt` (equivalent to vLLM's `_consolidate_system_messages`), so that becomes a single system message at position 0. Needed for downstream providers (e.g. vLLM) that don't support the `"developer"` role.
- Mid-conversation system messages: a system/developer message that appears *after* a user or assistant turn is not hoisted. It is decoded as an in-place `Message{Role: RoleSystem}` and re-emitted at its original index, because its position carries meaning ("from here on, answer in JSON") and hoisting also perturbs the cacheable token prefix. Anthropic and Gemini have no positional system role, so their encoders degrade `RoleSystem` to a `user` turn; OpenAI Responses emits a `developer` message item.
...
- **Handler retry loop** (`handler.go`): decodes IR once, retries by re-encoding per attempt. `retryLoopState.handleSendError` is the shared error-handling helper for both streaming and non-streaming paths. Context cancellation breaks the loop without retry. **Upstream error body forwarding**: when the inbound protocol matches the outbound protocol and the upstream returns an HTTP error, the upstream error body is forwarded directly to the client (preserving the exact error `code`, `type`, `message` structure from the upstream). For cross-protocol errors, `extractUpstreamErrorMessage` parses the upstream body to extract the clean error message, which is then formatted via the inbound codec's `WriteError`.
...
- **Gemini**: model is in URL path (not body); no native "tool" role; no explicit `tool_use` finish reason (infer from FunctionCall parts); streaming uses `streamGenerateContent?alt=sse`; ResponseFormat requires JSON Schema → Gemini Schema conversion; function calls carry no ID, so missing IDs are filled by `syntheticIDFor` — a **deterministic** sha256 over (name, args, per-name ordinal). This must stay deterministic: a random ID would change the replayed history on every request and destroy prompt-cache hits from the first tool call onward.
- **Prompt caching**: every protocol keys cache hits on a byte-identical request prefix, so conversion must be stable and order-preserving. Concretely: Anthropic `cache_control` breakpoints are carried through the IR via `ContentPart.BlockExtra` (allowlisted to `cache_control` only, and only re-emitted on an Anthropic target); interleaved `tool_result`/text within one user turn keeps its original order rather than hoisting tool results; mid-conversation system messages are not moved to position 0.
...
- **OpenAI Chat**: the leading run of `system`/`developer` roles → IR SystemPrompt, later ones → in-place `RoleSystem` messages; outbound emits `developer` role by default; `WithMapDeveloperToSystem` option emits `system` role instead (needed for downstream providers like vLLM that don't support the `developer` role); reads both `max_tokens` and `max_completion_tokens`; an assistant message always carries `content` or `tool_calls` (an empty string is emitted when every part was unrepresentable, since OpenAI rejects a message with neither, and `reasoning_content` is non-standard and does not satisfy the requirement)
